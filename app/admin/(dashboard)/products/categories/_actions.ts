...
export async function createCategory(
  ...
  try {
    ...
    await db.insert(productCategories).values({ slug, name, description });
...
export async function updateCategory(
  ...
  try {
    ...
    success = true;
    revalidatePath('/admin/products/categories');
    revalidatePath(`/admin/products/categories/${categoryId}/edit`);
  } catch (err: unknown) {
    const message =
  ...
  if (success) redirect('/admin/products/categories');
  return { ok: true };
}
